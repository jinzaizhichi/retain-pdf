...
fn normalize_failure_event(item: &mut JobEventRecord, resolved_failure: Option<&JobFailureInfo>) {
    let is_failure_event = matches!(
        item.event.as_str(),
        "failure_classified" | "job_terminal"
    ) || matches!(item.event_type.as_deref(), Some("failure_classified" | "job_terminal"));
    if !is_failure_event {
        return;
    }
    ...
    item.provider = failure.provider.clone().or_else(|| take_non_empty(item.provider.take()));
    item.provider_stage = failure
        .provider_stage
        .clone()
        .or_else(|| take_non_empty(item.provider_stage.take()));
    if item.stage_detail.as_deref().map(str::trim).unwrap_or("").is_empty() {
